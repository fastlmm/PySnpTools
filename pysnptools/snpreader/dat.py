...
class Dat(_OneShot, SnpReader):
    """
    A :class:`.SnpReader` for reading Dat/Fam/Map-formatted files from disk.
    ...
    **Methods beyond** :class:`.SnpReader`
    ...
    def __init__(self, filename, skiprows=0):
        ...
        self.filename = SnpReader._name_of_other_file(
            filename, remove_suffix="dat", add_suffix="dat"
        )
        self.skiprows = skiprows

    def _read_pstdata(self):
        row = SnpReader._read_fam(self.filename, remove_suffix="dat")
        col, col_property = SnpReader._read_map_or_bim(
            self.filename, remove_suffix="dat", add_suffix="map"
        )
        if len(row) == 0 or len(col) == 0:
            return SnpData(
                iid=row, sid=col, pos=col_property, val=np.empty([len(row), len(col)])
            )
        ...
        del datfields[2]
        assert (
            len(row) == datfields.shape[1]
        ), "Expect # iids in fam file to match dat file"
    ...
    def copyinputs(self, copier):
        ...
        copier.input(
            SnpReader._name_of_other_file(
                self.filename, remove_suffix="dat", add_suffix="dat"
            )
        )
        copier.input(
            SnpReader._name_of_other_file(
                self.filename, remove_suffix="dat", add_suffix="fam"
            )
        )
        copier.input(
            SnpReader._name_of_other_file(
    ...
    def write(filename, snpdata):
        ...
        SnpReader._write_map_or_bim(
            snpdata, filename, remove_suffix="dat", add_suffix="map"
        )
        filename = SnpReader._name_of_other_file(
            filename, remove_suffix="dat", add_suffix="dat"
        )
