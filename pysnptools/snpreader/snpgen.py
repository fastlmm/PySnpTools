...
class SnpGen(SnpReader):
    ...
    **Constructor:**
    ...
    def __init__(
        ...
        self._block_size = (
            block_size or sid_batch_size or max((100 * 1000) // max(1, iid_count), 1)
        )
        ...
        if cache_file is not None:
            if not os.path.exists(cache_file):
                ...
                with np.load(cache_file, allow_pickle=True) as data:
                    self._row = data["_row"]
                    assert (
                        len(self._row) == iid_count
                    ), "The iid in the cache file has a different length than iid_count"
                    self._col = data["_col"]
                    assert (
                        len(self._col) == sid_count
                    ), "The sid in the cache file has a different length than sid_count"
                    self._col_property = data["_col_property"]
                    assert (
                        len(self._col_property) == sid_count
                    ), "The pos in the cache file has a different length than sid_count"
                    self._ran_once = True

    def __repr__(self):
        return "{0}(seed={1},iid_count={2},sid_count={3},chrom_count={4},block_size={5},cache_file={6})".format(
    ...
    def _run_once(self):
        ...
        self._ran_once = True
        self._row = np.array(
            [("0", "iid_{0}".format(i)) for i in range(self._iid_count)]
        )
        ...
        for chrom_index in range(self._chrom_count):
            ...
            self._col_property[start:stop, 2] = (
                np.arange(0, stop - start) * step + random_increment + 1
            )
    ...
    def _read(
        ...
        row_index_count = (
            len(row_index_or_none) if row_index_or_none is not None else self._iid_count
        )  # turn to a count of the index positions e.g. all of them
        col_index = (
            col_index_or_none
            if col_index_or_none is not None
            else np.arange(self._sid_count)
        )  # turn to an array of index positions, e.g. 0,1,200,2200,10
        batch_index = (
            col_index // self._block_size
        )  # find the batch index of each index position, e.g. 0,0,0,2,0
        val = np.empty(
            (row_index_count, len(col_index)), order=order, dtype=dtype
        ...
        with log_in_place("working on snpgen batch", logging.INFO) as updater:
            for ii, i in enumerate(
                ...
                batch_val = self._get_val2(
                    start, stop, order=order, dtype=dtype
                ...
                val[:, a] = (
                    ...
                    else pstutil.sub_matrix(batch_val, row_index_or_none, b)
                )
    ...
    def _get_dist(self):
        ...
        x_sample = np.logspace(
            np.log10(0.1 / self._iid_count), np.log10(0.5), 100, base=10
        )  # discretize from 1e-7 (assuming iid_count=1e6) to .5, logarithmically
        y_sample = np.array(
            [dist_fit(x) for x in x_sample]
        )  # Find the relative weight of each point
    ...
    def _get_val2(self, sid_start, sid_stop, order, dtype):
        ...
        (
            ...
            self._get_dist()
        )  # The discrete distribution of minor allele frequencies (based on curve fitting to real data)
        val = SnpGen._get_val(
            x_sample,
            dist,
            self._iid_count,
    ...
    def _get_val(x_sample, dist, iid_count, sid_start, sid_stop, seed, order, dtype):
        ...
        while sid_so_far < block_size_a:
            ...
            sid_index_to_freq = np.random.choice(
                x_sample, size=block_size_b, replace=True, p=dist
            )  # For each sid, pick its minor allele freq
            val_b = (
                (np.random.rand(iid_count, 2, block_size_b) < sid_index_to_freq)
                .sum(axis=1)
                .astype(dtype)
            )  # Sample each allele and then sum the minor alleles # !!!slowest part
            missing = np.random.rand(iid_count, block_size_b) < missing_rate
            val_b[missing] = np.nan

            sid_index_to_minor_allele_count = np.nansum(
                val_b, axis=0
            )  # Find the # of minor alleles for each allele. By design this will be mostly 0's

            # Keep only the sids with a minor allele count between 1 and iid_count.
            legal = (sid_index_to_minor_allele_count != 0) * (
                sid_index_to_minor_allele_count <= iid_count
            )
            legal_count = legal.sum()
            val_a[:, sid_so_far : sid_so_far + legal_count] = val_b[:, legal][
                :, : block_size_a - sid_so_far
            ]  # !!! takes about 1/3 of the time. Faster if both were order="F"?
...
class TestSnpGen(unittest.TestCase):
    ...
    def test1(self):
        ...
        seed = 0
        snpgen = SnpGen(
            seed=seed, iid_count=1000, sid_count=1000 * 1000, block_size=1000
        )
