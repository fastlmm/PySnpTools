    ...
    **Constructor:**
    ...
    def _run_once(self):
        ...
        with self._storage.open_read(_metadatanpz) as handle_metadatanpz_file_name:
            ...
            with self._storage.open_read(
                _reader_name_listnpz
            ) as handle_reader_name_listnpz_file_name:
                reader_name_list = np.array(
                    np.load(handle_reader_name_listnpz_file_name)["reader_name_list"],
                    dtype="str",
                )
                # self._file_dict["reader_name_listnpz"] = handle_reader_name_listnpz

                reader_list = [
                    _Distributed1Bed(reader_name, self._storage)
                    for reader_name in reader_name_list
                ]

                self._merge = _MergeSIDs(
                    reader_list,
                    cache_file=handle_metadatanpz_file_name,
                    skip_check=True,
                )
    ...
    @staticmethod
    def write(
        storage, snpreader, piece_per_chrom_count=1, updater=None, runner=None
        ...
        with _file_transfer_reporter(
            "DistributedBed.write", size=0, updater=updater
        ) as updater2:
            ...
            def mapper_closure(chrom):
                ...
                def nested_closure(piece_per_chrom_index):
                    start = (
                        chrom_reader.sid_count
                        * piece_per_chrom_index
                        // piece_per_chrom_count
                    )
                    stop = (
                        chrom_reader.sid_count
                        * (piece_per_chrom_index + 1)
                        // piece_per_chrom_count
                    )
                    ...
                    if (
                        sum(exist_list) < 3
                    ):  # If all three of the BIM/FAM/BED files are already there, then skip the upload, otherwise do the upload
                        for i in range(
                            3
                        ):  # If one or two of BIM/FAM/BED are there, remove them
                            if exist_list[i]:
                                storage.remove(_piece_name_list[i])
                        _Distributed1Bed.write(
                            _piece_name_list[-1],
                            storage,
                            piece_reader.read(),
                            count_A1=count_A1,
        ...
        with storage.open_write(_metadatanpz) as local_metadatanpz:
            ...
            with storage.open_write(_reader_name_listnpz) as local_reader_name_listnpz:
                ...
                np.savez(
                    local_reader_name_listnpz, reader_name_list=reader_name_list_ascii
                )
...
class _Distributed1Bed(SnpReader):
    ...
    def __repr__(self):
        return "{0}('{1}','{2}')".format(
            self.__class__.__name__, self.path, self._storage
        )
    ...
    def row(self):
        ...
        if not hasattr(self, "_row"):
            _fam = SnpReader._name_of_other_file(
                self.path, remove_suffix="bed", add_suffix="fam"
            )
    ...
    def col(self):
        ...
        if not hasattr(self, "_col"):
            _bim = SnpReader._name_of_other_file(
                self.path, remove_suffix="bed", add_suffix="bim"
            )
    ...
    def _run_once(self):
        ...
        _bed = SnpReader._name_of_other_file(
            self.path, remove_suffix="bed", add_suffix="bed"
        )
    ...
    def write(path, storage, snpdata, count_A1=True, updater=None):
        file_list = [
            SnpReader._name_of_other_file(
                path, remove_suffix="bed", add_suffix=new_suffix
            )
            for new_suffix in ["bim", "fam", "bed"]
        ]  #'bed' should be last
        with _multiopen(
            lambda file_name: storage.open_write(file_name, updater=updater), file_list
        ) as local_file_name_list:
...
class TestDistributedBed(unittest.TestCase):
    def test1(self):
        ...
        distributed_bed = DistributedBed.write(
            temp_dir, snpgen, piece_per_chrom_count=2
        )
        snpdata = distributed_bed.read()

        ref1 = DistributedBed(
            os.path.dirname(os.path.realpath(__file__))
            + "/../../tests/datasets/distributed_bed_test1"
        ).read()
        assert snpdata.allclose(ref1, equal_nan=True)

        ref2 = Bed(
            os.path.dirname(os.path.realpath(__file__))
            + "/../../tests/datasets/distributed_bed_test1_X.bed",
            count_A1=False,
        ).read()
...
if __name__ == "__main__":
    ...
    if False:
        ...
        if os.path.exists(directory):
            shutil.rmtree(directory)
        snpreader = Bed("../examples/toydata.5chrom.bed", count_A1=False)[
            :, ::10
        ]  # Read every 10 snps from Bed format
        DistributedBed.write(
            directory, snpreader, piece_per_chrom_count=5
        )  # Write data in DistributedBed format
