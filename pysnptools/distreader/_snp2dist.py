...
class _Snp2Dist(DistReader):
    ...
    def _snpval_to_distval(self, snpval, order, dtype):
        ...
        distval = np.zeros(
            [snpval.shape[0], snpval.shape[1], 3], dtype=dtype, order=order
        )
        ...
        distval[
            distval.sum(axis=-1) != 1, :
        ] = np.nan  # replace any dist that doesn't sum to 1 with nan
        distval = distval.reshape(
            [snpval.shape[0], snpval.shape[1], distval.shape[-1]], order=order
        )
        return distval
    ...
    def _read(
        ...
        assert (
            row_index_or_none is None and col_index_or_none is None
        )  # real assert because indexing should already be pushed to the inner snpreader
        dtype = np.dtype(dtype)
        ...
        if self.block_size is None or (
            self.sid_count <= self.block_size or self.sid_count <= self.iid_count
        ):
            ...
            if order == "A":
                order = "F"
            val = np.zeros(
                [self.iid_count, self.sid_count, 3], dtype=dtype, order=order
            )  # LATER use empty or fillnan???

            logging.info(
                "reading {0} value data in blocks of {1} SNPs and finding distribution (for {2} individuals)".format(
                    self.sid_count, self.block_size, self.iid_count
                )
            )
            ct = 0
            ts = time.time()
            ...
            for start in range(0, self.sid_count, self.block_size):
                ...
                val[:, start : start + self.block_size] = self._snpval_to_distval(
    ...
    def __getitem__(self, iid_indexer_and_snp_indexer):
        ...
        return _Snp2Dist(
            self.snpreader[row_index_or_none, col_index_or_none],
