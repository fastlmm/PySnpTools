...
class PstMemMap(PstData):
    ...
    **Methods beyond** :class:`PstReader`
    ...
    def empty(
        ...
        self._empty_inner(
            row, col, filename, row_property, col_property, order, dtype, val_shape
        )
        return self

    def _empty_inner(
        self, row, col, filename, row_property, col_property, order, dtype, val_shape
    ):
        self._ran_once = True
        ...
        shape = (
            ...
            else (len(row), len(col), val_shape)
        )
        val = np.memmap(
            filename,
            offset=self._offset,
        ...
        logging.info(
            "Finished allocating memmap '{0}'. Size is {1:,}".format(
        ...
        PstData.__init__(
            ...
            row_property,
            col_property,
            name="np.memmap('{0}')".format(filename),
        )
    ...
    def write(filename, pstdata):
        ...
        self = PstMemMap.empty(
            pstdata.row,
            pstdata.col,
            filename + ".temp",
            row_property=pstdata.row_property,
            col_property=pstdata.col_property,
            order=PstMemMap._order(pstdata),
            dtype=pstdata.val.dtype,
            val_shape=pstdata.val_shape,
        )
...
class TestPstMemMap(unittest.TestCase):
    def test1(self):
        ...
        assert np.array_equal(
            pstreader2[[0], [0]].read(view_ok=True).val, np.array([[1.0]])
        )
        pstreader2.flush()
        assert isinstance(pstreader2.val, np.memmap)
        assert np.array_equal(
        ...
        assert np.array_equal(
            pstreader3[[0], [0]].read(view_ok=True).val, np.array([[1.0]])
        )
