...
class Hadoop2(Runner):
    ...
    def __init__(
        ...
        self.xmx = xmx
        self.yarn_scheduler_minimum_allocation_mb = (
            min_alloc  # yarn.scheduler.minimum-allocation-mb=2048
        )
    ...
    def run(self, distributable):
        ...
        remotepythonpath, tgzListPythonPath = self.FindOrCreateRemotePythonPath(
            localpythonpath, remotewd
        )

        tgzList = []
        inputOutputCopier = HadoopCopier(
            remotewd, fileInWorkingDirectoryList, tgzList, self.skipdatacheck
        )  # Create the object that copies input and output files to where they are needed
        inputOutputCopier.input(
            distributable
        )  # copy of the input files to where they are needed (i.e. the cluster)

        batfilename_abs_list = self.create_bat_file(
            distributable,
            remotepythonpath,
            remotewd,
            run_dir_abs,
        ...
        inputOutputCopier.output(
            distributable
        )  # copy the output file from where they were created (i.e. the cluster) to the local computer

        subprocess.check_output(
            "%HADOOP_HOME%\\bin\\hadoop fs -copyToLocal {0} {1}\n".format(
                result_hdfs, result_local.replace("\\", "/")
            ),
    ...
    def submit_to_cluster(
        ...
        s00 = r"%HADOOP_HOME%\bin\hadoop fs -rm -r -f -skipTrash {0}".format(
            run_dir_rel.replace("\\", "/")
        )
        s0 = r"%HADOOP_HOME%\bin\hadoop fs -copyFromLocal {0} {1}".format(
            run_dir_rel.replace("\\", "/"), run_dir_rel.replace("\\", "/") + "/.."
        )
        ...
        if self.xmx is None:
            ...
            xmx_string = "\n        -D mapreduce.map.java.opts=-Xmx{0}m ^".format(
                self.xmx
            )
        ...
        if self.yarn_scheduler_minimum_allocation_mb is None:
            ...
            min_alloc_string = (
                "\n     -D yarn.scheduler.minimum-allocation-mb={0} ^".format(
                    self.yarn_scheduler_minimum_allocation_mb
                )
            )
        ...
        with TemporaryFile() as output:
            ...
            stderr0 = output.read()
        logging.info(
            "Result from 'Hadoop2 runner running the copyFromLocal' is stdout='{0}', stderr='{1}'".format(
                stdout0, stderr0
            )
        )
        if stderr0 != "":
        ...
        with TemporaryFile() as output:
            ...
            stderr = output.read()
        logging.info(
            "Result from 'Hadoop2 runner running the streamingjar' is stdout='{0}', stderr='{1}'".format(
                stdout, stderr
            )
        )
    ...
    def FindOrCreatePythonSettings(self, remotewd):
        ...
        tgzName = HadoopCopier.CheckUpdateTgz(
            localpythonpathsetting,
            subsubItemList1=None,
            skipcheck=False,
            filter_hidden=False,
        )
    ...
    def create_bat_file(
        ...
        distributable_py_file = os.path.join(
            os.path.dirname(__file__), "..", "distributable.py"
        )
        if not os.path.exists(distributable_py_file):
            raise Exception(
                "Expect file at " + distributable_py_file + ", but it doesn't exist."
            )
        ...
        for part in ["Mapper", "Reducer"]:
            command_string = (
                ...
                + os.path.sep
                + file
                + r""" distributable.p "Local{0}({1},""{2}"",mkl_num_threads={3},logging_handler=logging.StreamHandler())" """.format(
                    part,
                    self.taskcount,
                    result_remote.replace(
                        "\\", "/"
                    ),  # change to dos seperater to unix seperater because python will work with either and this avoid problems with parsing the batch file
                    self.mkl_num_threads,
                )
            )
            batfilename_rel = os.path.join(run_dir_rel, "dist{0}.bat".format(part))
            batfilename_abs = "hdfs:" + os.path.join(
            ...
            with open(batfilename_rel, "w") as batfile:
                ...
                if part == "Reducer":
                    batfile.write(
                        "@call %HADOOP_HOME%\\bin\\hadoop fs -rm -skipTrash -f {0}\n".format(
                            result_hdfs
                        )
                    )
                    batfile.write(
                        "@call %HADOOP_HOME%\\bin\\hadoop fs -copyFromLocal {0} {1}\n".format(
                            result_remote.replace("\\", "/"), result_hdfs
                        )
                    )
                    for outfile in outFileList:
                        hdfsOutFile = remotewd + "/" + outfile
                        batfile.write(
                            "@call %HADOOP_HOME%\\bin\\hadoop fs -rm -f {0}\n".format(
                                hdfsOutFile
                            )
                        )
                        batfile.write(
                            "@call %HADOOP_HOME%\\bin\\hadoop fs -copyFromLocal {0} {1}\n".format(
                                outfile.replace("\\", "/"), hdfsOutFile
                            )
                        )
        picklefilename_abs = "hdfs:" + os.path.join(
            run_dir_abs, "distributable.p"
        ).replace("\\", "/")
    ...
    def FindOrCreateRemotePythonPath(self, localpythonpath, remotewd):
        ...
        for i, localpythonpathdir in enumerate(localpythonpath.split(";")):
            tgzName = HadoopCopier.CheckUpdateTgz(
                localpythonpathdir, skipcheck=self.skipsourcecheck
            )
            lastFolderName = os.path.split(os.path.normpath(localpythonpathdir))[1]
            hdfstgz = "hdfs:{3}/pythonpath.{0}.{2}.{1}/{2}.tgz".format(
                i,
                str(datetime.datetime.fromtimestamp(os.path.getmtime(tgzName)))[:19]
                .replace(" ", "_")
                .replace(":", "_"),
    ...
    def hdfsCopyFromLocalIfNotThere(tgzName, hdfstgz):
        ...
        try:
            subprocess.check_output(
                r"type {0} | %HADOOP_HOME%\bin\Hadoop fs -put -f - {1}".format(
                    tgzName, hdfstgz[5:]
                ),
                stderr=subprocess.STDOUT,
                shell=True,
    ...
    def hadoop_command(command_string):
        ...
        if rc != 0:
            raise Exception(
                "Hadoop command '{0}' fails with rc={1}.".format(command_string, rc)
            )
...
class ListCopier(object):  # Implements ICopier
    def __init__(self, inputList, outputList):  # The list will be modified
        if len(inputList) != 0:
            raise Exception("Expect inputList to start empty")
        if len(outputList) != 0:
            raise Exception("Expect outputList to start empty")
...
class HadoopCopier(object):  # Implements ICopier
    def __init__(
        self, remotewd, fileInWorkingDirectoryList, tgzList, skipdatacheck
    ):  # The two lists will be modified
        if len(fileInWorkingDirectoryList) != 0:
            raise Exception("Expect fileInWorkingDirectoryList to start empty")
        if len(tgzList) != 0:
            raise Exception("Expect tgzList to start empty")
    ...
    def input(self, item):
        ...
        (
            fileInWorkingDirectoryList,
            subDirectoryToSubSubItemList,
        ) = self.GroupByTopLevelSubFolder(inputList)

        # create or update a tgz for each directory
        for directory, subsubItemList in subDirectoryToSubSubItemList.iteritems():
            tgzName = HadoopCopier.CheckUpdateTgz(
                directory, subsubItemList1=subsubItemList, skipcheck=self.skipdatacheck
            )
            hdfsName = "{0}/{1}.{2}.tgz".format(
                self.remotewd,
                directory,
                str(datetime.datetime.fromtimestamp(os.path.getmtime(tgzName)))[:19]
                .replace(" ", "_")
            ...
            hdfsNameWild = "{0}/{1}.{2}.tgz".format(
                self.remotewd, directory, "*"
            ).replace("\\", "/")
            try:
                subprocess.check_output(
                    r"%HADOOP_HOME%\bin\Hadoop fs -ls {0}".format(hdfsName),
                    stderr=subprocess.STDOUT,
                    shell=True,
                )
            except Exception:
                ...
                subprocess.check_output(
                    r"%HADOOP_HOME%\bin\Hadoop fs -copyFromLocal {0} {1}".format(
                        tgzName, hdfsName
                    ),
                    stderr=subprocess.STDOUT,
                    shell=True,
        ...
        for file in fileInWorkingDirectoryList:
            if os.path.getsize(file) > 10 * 1024 * 1024:
                logging.warn(
                    "File '{0}' is large ({1}GB) and would be more efficiently distributed on Hadoop if placed in subfolder of the working directory.".format(
                        file, round(os.path.getsize(file) / (1024 * 1024))
                    )
                )
    ...
    def CheckUpdateTgz(
        ...
        if skipcheck and os.path.exists(tgzName):
            logging.info(
                "skipcheck is True and '{0}' is there, so skipping".format(tgzName)
            )
            return tgzName
        ...
        if os.path.exists(tgzName) and not [] in subsubItemList1 and not filter_hidden:
            ...
            for tarinfo in tarfile.getmembers():
                if tarinfo.isfile():
                    filenamein = tarinfo.filename.replace(
                        "/", os.path.sep
                    )  # "/" is the tar separator
                    filename = os.path.join(directory, filenamein)
                    if os.path.exists(filename) and os.path.isfile(filename):
                        subsubItemList1.append(
                            filenamein.split("/")
                        )  # "/" is the tar separator
            tarfile.close()
        ...
        if not HadoopCopier.IsTarFileUpToDate(
            directory, subsubItemListOut, tgzName, filter_hidden
        ):
            HadoopCopier.CreateNewTarFile(
                directory, subsubItemListOut, tgzName, filter_hidden
            )
        return tgzName
    ...
    def CreateNewTarFile(directory, subsubItemList, tgzName, filter_hidden=True):
        ...
        tgzNameTemp = (
            tgzName + ".tar"
        )  # create a temp file so that both files will exist for a while
        ...
        for subsubItem in subsubItemList:
            ...
            tarfile.add(
                os.path.normpath(directory + "/" + tarName),
                tarName,
                recursive=True,
                filter=lambda x, directory1=directory1, filter_hidden=filter_hidden: HadoopCopier.tarfile_filter_hidden(
                    x, directory1, filter_hidden
                ),
            )
        tarfile.close()
        logging.info("Compressing '{0}'".format(tgzNameTemp))
        subprocess.call(
            r"c:\cygwin64\bin\gzip.exe --force --fast {0}".format(tgzNameTemp),
            shell=True,
        )
    ...
    def GroupByTopLevelSubFolder(self, inputList):
        ...
        for item in inputList:
            ...
            if not os.path.exists(relpath):
                raise Exception(
                    "Expect input file (or directory) '{0}' but it doesn't exist".format(
                        relpath
                    )
                )
            ...
            if os.path.isfile(relpath):  # If the input item is a file ...
                if len(parts) == 1:  # If the input item is in the working directory
                    filesInWorkingDirectory.append(
                        relpath
                    )  # add it to the filesInWorkingDirectory list
                else:  # A file in a subfolder
                    subDirectoryToSubSubItemList[parts[0]].append(
                        parts[1:]
                    )  # Add it to the list for the subfolder
            else:  # A folder
    ...
    def IsTarFileUpToDate(directory, subsubItemList, tgzName, filter_hidden):
        if os.path.exists(tgzName):
            ...
            if not os.path.isfile(tgzName):
                raise Exception("Expect '{0}' to be a file.".format(tgzName))
            tarfile = tarfileLibrary.open(tgzName, "r")
            isUpToDate = HadoopCopier.IsTarFileUpToDateInternal(
                directory, subsubItemList, tarfile, filter_hidden
            )
            tarfile.close()
    ...
    def IsTarFileUpToDateInternal(directory, subsubItemList, tgzFile, filter_hidden):
        howToIgnoreString = "To ignore changes in a directory add a file '{0}'".format(
            HadoopCopier._ignoreTgzChangeFileName
        )

        for subsubItem in subsubItemList:
            tarName = "/".join(
                subsubItem
            )  # use "/" instead of os.path.sep because that is what the tar file uses
            winfileOrDirectory = os.path.normpath(directory + os.path.sep + tarName)
            try:
                member = tgzFile.getmember(tarName)
            except Exception as e:
                logging.info(
                    "'{0}' not up to date because of exception {1}. ({2})".format(
                        tarName, e, howToIgnoreString
                    )
                )
                return False
            else:
                if os.path.isfile(winfileOrDirectory):
                    if int(os.path.getmtime(winfileOrDirectory)) != int(member.mtime):
                        logging.info(
                            "'{0}' not up to date because of date change in '{1}'. ({2})".format(
                                tgzFile, winfileOrDirectory, howToIgnoreString
                            )
                        )
                        return False
                else:
                    if not os.path.isdir(winfileOrDirectory):
                        raise Exception(
                            "Expect '{0}' to be a directory. ({1})".format(
                                winfileOrDirectory, howToIgnoreString
                            )
                        )
                    for winfileOrDirectory2 in HadoopCopier.WalkWithoutHidden(
                        winfileOrDirectory, filter_hidden
                    ):
                        tarName2 = os.path.relpath(
                            winfileOrDirectory2, winfileOrDirectory
                        ).replace("\\", "/")
                        try:
                            member = tgzFile.getmember(tarName2)
                        except KeyError:
                            logging.info(
                                "'{0}' not up to date because '{1}' is not found. ({2})".format(
                                    tgzFile, tarName2, howToIgnoreString
                                )
                            )
                            return False
                        else:
                            if os.path.isfile(winfileOrDirectory2):
                                if int(os.path.getmtime(winfileOrDirectory2)) != int(
                                    member.mtime
                                ):
                                    logging.info(
                                        "'{0}' not up to date because of date change in '{1}'. ({2})".format(
    ...
    def is_hidden(filepath, filter_hidden):
        if not filter_hidden:
            ...
            return (
                name.startswith(".")
                or HadoopCopier.has_hidden_attribute(filepath)
                or HadoopCopier.containsDotIgnoreTgzChange(filepath)
            )
