...
class Hadoop(Runner):
    ...
    def submit_to_cluster(
        ...
        with TemporaryFile() as output:
            ...
            stderr0 = output.read()
        logging.info(
            "Result from 'Hadoop runner running the copyFromLocal' is stdout='{0}', stderr='{1}'".format(
                stdout0, stderr0
            )
        )
        if stderr0 != "":
        ...
        with TemporaryFile() as output:
            ...
            stderr = output.read()
        logging.info(
            "Result from 'Hadoop runner running the streamingjar' is stdout='{0}', stderr='{1}'".format(
                stdout, stderr
            )
        )
    ...
    def create_bat_file(
        ...
        for part in ["Mapper", "Reducer"]:
            command_string = (
                ...
                + r""" distributable.p "Local{0}({1},""{2}"",mkl_num_threads={3},logging_handler=logging.StreamHandler())" """.format(
                    ...
                    result_remote.replace(
                        "\\", "/"
                    ),  # change to DOS separator to Unix separator because python will work with either and this avoid problems with parsing the batch file
            ...
            with open(batfilename_rel, "w") as batfile:
                ...
                if part == "Reducer":
                    batfile.write(
                        "@call %HADOOP_HOME%\\bin\\hadoop fs -rm {0} -skipTrash\n".format(
                            result_hdfs
                        )
                    )
                    batfile.write(
                        "@call %HADOOP_HOME%\\bin\\hadoop fs -copyFromLocal {0} {1}\n".format(
                            result_remote, result_hdfs
                        )
                    )
                    for outfile in outFileList:
                        hdfsOutFile = remotewd + "/" + outfile
                        batfile.write(
                            "@call %HADOOP_HOME%\\bin\\hadoop fs -rm {0}\n".format(
                                hdfsOutFile
                            )
                        )
                        batfile.write(
    ...
    def hdfsCopyFromLocalIfNotThere(tgzName, hdfstgz):
        ...
        subprocess.check_output(
            r"type {0} | %HADOOP_HOME%\bin\Hadoop fs -put - {1}".format(
                tgzName, hdfstgz[5:]
            ),
            stderr=subprocess.STDOUT,
            shell=True,
...
class HadoopCopier(object):  # Implements ICopier
    ...
    def input(self, item):
        ...
        for directory, subsubItemList in subDirectoryToSubSubItemList.iteritems():
            ...
            lookout = subprocess.check_output(
            ...
            if "No such file or directory" in lookout:
                subprocess.check_output(
                    r"%HADOOP_HOME%\bin\Hadoop fs -rm {0}".format(hdfsNameWild),
                    stderr=subprocess.STDOUT,
                    shell=True,
                )
                subprocess.check_output(
