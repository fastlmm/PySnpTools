...
def create_task_file_name(workdirectory, taskindex, taskcount):
    ...
    task_file_name = os.path.join(
        workdirectory, "{0}.{1}.p".format(taskindex, taskcount)
    )
...
def _run_one_task(
    ...
    with patch.dict("os.environ", environ if environ is not None else {}) as _:
        if taskindex < taskcount:
            doMainWorkForOneIndex(
                shaped_distributable, taskcount, taskindex, workdirectory
            )
            return None
        else:
...
def _work_sequence_for_one_index(distributable, taskAndWorkcount, taskindex):
    if hasattr(distributable, "work_sequence_range"):
        ...
        for workIndex, work in enumerate(distributable.work_sequence()):
            if workIndex == taskAndWorkcount:
                raise Exception(
                    "Expect len(work_sequence) to match work_count, but work_sequence was too long"
                )
            if workIndex == taskindex:
                yield work
                workDone = True
                if (
                    workIndex != taskAndWorkcount - 1
                ):  # the work is done, so quit enumerating work (but don't quit early if you're the last workIndex because we want to double check that the work_sequence and work_count match up)
                    break
        if not workDone:
            raise Exception(
                "Expect len(work_sequence) to match work_count, but work_sequence was too short"
            )
...
def doMainWorkForOneIndex(distributable, taskAndWorkcount, taskindex, workdirectory):
    ...
    with open(task_file_name, mode="wb") as f:
        for work in _work_sequence_for_one_index(
            ...
            result = _run_all_in_memory(work)
            cloudpickle.dump(result, f)  # save a result to the temp results file
...
def work_sequence_from_disk(workdirectory, taskAndWorkcount):
    ...
    for taskindex in range(taskAndWorkcount):
        task_file_name = create_task_file_name(
            workdirectory, taskindex, taskAndWorkcount
        )
        with open(task_file_name, mode="rb") as f:
            try:
                ...
                raise Exception(
                    "Error trying to unpickle '{0}'. {1}".format(task_file_name, detail)
                )
        # if True:
...
class BatchUpWork(object):  # implements IDistributable
    ...
    def __init__(self, distributable, workcount, taskcount, weights=None):
        ...
        if taskcount <= 0:
            raise Exception("Expect taskcount to be at least 1.")

        if weights is None:
            self._partial_sum_weight = list(range(0, taskcount + 1))
        else:
            if len(weights) != taskcount:
                raise Exception("Expect weights to have length of taskcount.")
            self._partial_sum_weight = [0]
            for weight in weights:
                if weight != int(weight) or weight < 0:
                    raise Exception("Expect weights be non-negative integers.")
                self._partial_sum_weight.append(weight + self._partial_sum_weight[-1])
        if self._partial_sum_weight[-1] == 0:
            raise Exception("Weights cannot be all zero.")
    ...
    def pull_sub_work(self, start, stop):
        if hasattr(self.sub_distributable, "work_sequence_range"):
            ...
            for sub_workIndex, sub_work in enumerate(
                self.sub_distributable.work_sequence()
            ):
                if sub_workIndex == self.sub_workcount:
                    raise Exception("Expect len(work_sequence) to match work_count")
                if sub_workIndex == sub_workIndexList[-1]:
                    yield sub_workIndex, sub_work
                    sub_workIndexList.pop()
                    if (
                        len(sub_workIndexList) == 0
                        and sub_workIndex != self.sub_workcount - 1
                    ):  # there are no more work items for this task, so quit (but don't quit early if you're the last workIndex because we want to double check that the work_sequence and work_count match up)
                        break
            if len(sub_workIndexList) != 0:
                raise Exception(
                    "no work was enumerated for workIndex={0}".format(
                        sub_workIndexList[-1]
                    )
                )
    ...
    def create_sub_result_sequence(self, result_sequence):
        for result in result_sequence:
            ...
            if (stop - start) != len(result):
                raise Exception("Assert: batched results not expected size")
            for sub_workIndex, pair in zip(range(start, stop), result):
                sub_workIndex_check, sub_result = pair
                if sub_workIndex != sub_workIndex_check:
                    raise Exception("Assert: Unexpected workindex in batched result")
                yield sub_result
    ...
    def createSubWorkIndexList(self, workindex):
        ...
        start = (
            self._partial_sum_weight[workindex]
            * self.sub_workcount
            // self._partial_sum_weight[-1]
        )  # assuming high precision integer math
        stop = (
            self._partial_sum_weight[workindex + 1]
            * self.sub_workcount
            // self._partial_sum_weight[-1]
        )  # assuming high precision integer math
        return start, stop
...
class ExpandWork(object):  # implements IDistributable
    def __init__(self, distributable, workcount, taskcount):
        ...
        assert (
            workcount < taskcount
        ), "ExpandWork expects workcount to be less than taskcount"
        self.sub_distributable = distributable
        self.sub_workcount = workcount
        self._workcount = taskcount
        self.floor = taskcount // workcount  # all work gets this many tasks
        self.excess = (
            taskcount - self.floor * workcount
        )  # but there are this many extra tasks
        self.extra_index = (
            self.floor + 1
        ) * self.excess  # tasks before here, get one extra task
    ...
    def _work_sequence_range(self, start, stop):
        ...
        for sub_work in self.sub_distributable_work_sequence_range(sub_start, sub_stop):
            ...
            sub_sub_start, sub_sub_stop = self.sub_sub_start_stop(
                sub_workIndex, start, stop
            )
            if not callable(
                sub_work
            ):  # i.e. a distributable job. If our expand_to is 3 and i's work_count is 9, then it is batched up 3x3. On the other hand, if its work_count is 2, then we ExpandWork from 2 to 3.
                shaped_distributable = _shape_to_desired_workcount(sub_work, expand_to)
                for sub_sub_work in shaped_distributable.work_sequence_range(
                    sub_sub_start, sub_sub_stop
                ):
                    yield sub_sub_work
                    workIndex += 1
                #    expand_to -= 1
...
class MakeWork(object):  # implements IDistributable
    ...
    def reduce(self, result_sequence):
        for (
            result
        ) in result_sequence:  # We iterate through, just so everything is accounted for
            pass
