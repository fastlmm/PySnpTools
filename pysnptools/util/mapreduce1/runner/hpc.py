...
class HPC(Runner):
    ...
    def __init__(
        ...
        update_remote_python_parent=False,
        min=None,
        max=None,
        excluded_nodes=[],
        template=None,
    ...
    def run(self, distributable):
        ...
        pstutil.create_directory_if_necessary(
            os.path.join(remotewd, distributable.tempdirectory), isfile=False
        )  # create temp directory now so that cluster tasks won't try to create it many times at once
        result_remote = os.path.join(run_dir_abs, "result.p")

        self.copy_python_settings(run_dir_abs)

        inputOutputCopier = HPCCopier(
            remotewd, skipinput=self.skipinputcopy
        ...
        remotepythonpath = self.FindOrCreateRemotePythonPath(
            localpythonpath, run_dir_abs
        )

        batfilename_rel = self.create_bat_file(
            distributable,
            remotepythoninstall,
            remotepythonpath,
            remotewd,
        ...
        assert os.path.exists(
            result_remote
        ), "The HPC job produced no result (and, thus, likely failed)"
    ...
    def CheckUnitAndMKLNumThreads(self, mkl_num_threads, unit):
        if unit.lower() == "core":
            if mkl_num_threads is not None and mkl_num_threads != 1:
                raise Exception(
                    "When 'unit' is 'core', mkl_num_threads must be unspecified or 1"
                )
            self.mkl_num_threads = 1
        elif unit.lower() == "socket":
            if mkl_num_threads is None:
                raise Exception(
                    "When 'unit' is 'socket', mkl_num_threads must be specified"
                )
    ...
    def copy_python_settings(self, run_dir_abs):
        ...
        if os.path.exists(python_settings):
            ...
            remote_user_python_settings = os.path.join(
                run_dir_abs, user_python_settings
            )
    ...
    def FindOrCreateRemotePythonPath(self, localpythonpath, run_dir_abs):
        if self.remote_python_parent is None:
            ...
            pstutil.create_directory_if_necessary(
                self.remote_python_parent, isfile=False
            )
            list = []
    ...
    def submit_to_cluster(
        ...
        if len(self.excluded_nodes) > 0:
            excluded_nodes = "Set-HpcJob -Id $r.Id -addExcludedNodes {0}".format(
        ...
        """.format(
                    ...
                    self.numString(),  # 12
                    excluded_nodes,  # 13
                    ' -templateName "{0}"'.format(self.template)
                    if self.template is not None
                    else "",  # 14
                    self.runtime,  # 15 RuntimeSeconds
                    ' -NodeGroups "{0}"'.format(self.nodegroups)
        ...
        if not 0 == proc.wait():
            raise Exception(
                "Running powershell cluster submit script results in non-zero return code"
            )
    ...
    def FindDirectoriesToExclude(localpythonpathdir):
        logging.info(
            "Looking in '{0}' for directories to skip".format(localpythonpathdir)
        )
    ...
    def CopySource(self, localpythonpath, run_dir_abs):
        ...
        for i, localpythonpathdir in enumerate(localpythonpath.split(";")):
            ...
            xcopycommand = "robocopy /s {0} {1}{2}".format(
                localpythonpathdir, remotepythonpathdir, xd_string
            )
    ...
    def create_bat_file(
        ...
        for path_share in path_share_list:
            ...
            if not os.path.isdir(path_share_abs):
                raise Exception("Expect path directory at '{0}'".format(path_share_abs))
            remotepath_list.append(path_share_abs)
        remotepath = ";".join(remotepath_list)

        (
            distributablep_filename_rel,
            distributablep_filename_abs,
        ) = self.create_distributablep(distributable, run_dir_abs, run_dir_rel)
        ...
        for remote_path_part in remotepythonpath.split(";"):
            ...
            remoteexe = None
        assert (
            remoteexe is not None
        ), "Could not find '{0}' on remote python path. Is fastlmm on your local python path?".format(
            file
        )
        ...
        command_string = (
            remoteexe
            + r""" "{0}" """.format(distributablep_filename_abs)
            + r""" "LocalInParts(%1,{0},mkl_num_threads={1},result_file=""{2}"",run_dir=""{3}"") " """.format(
                self.taskcount,
        ...
        if self.node_local:
            with open(os.path.join(run_dir_abs, "nodeprep.bat"), "w") as prepfile:
                ...
                with open(
                    os.path.join(run_dir_abs, "noderelease.bat"), "w"
                ) as releasefile:
                    releasefile.write(r"""set f="{0}"{1}""".format(remotewd, "\n"))
                    releasefile.write(r"""set t="{0}"{1}""".format(nodelocalwd, "\n"))
                    inputOutputCopier = HPCCopierNodeLocal(
                        prepfile, releasefile, self.clean_up
                    )  # Create the object that copies input and output files to where they are needed
                    inputOutputCopier.input(
                        distributable
                    )  # copy of the input files to where they are needed (i.e. to the cluster)
                    inputOutputCopier.output(
                        distributable
                    )  # copy of the output files to where they are needed (i.e. off the cluster)
    ...
    def check_remote_pythoninstall(self):
        ...
        if not os.path.isdir(remotepythoninstall):
            raise Exception(
                "Expect Python and related directories at '{0}'".format(
                    remotepythoninstall
                )
            )
...
class HPCCopier(object):  # Implements ICopier
    ...
    def input(self, item):
        ...
        if isinstance(item, str):
            ...
            xcopycommand = "xcopy /d /e /s /c /h /y {0} {1}".format(
                itemnorm, remote_dir_name
            )
            logging.info(xcopycommand)
            ...
            if rc != 0:
                raise Exception(
                    "xcopy cmd failed with return value={0}, from cmd {1}".format(
                        rc, xcopycommand
                    )
                )
    ...
    def output(self, item):
        if isinstance(item, str):
            ...
            assert os.path.exists(
                remote_file_name
            ), "Don't see expected file '{0}'. Did the HPC job fail?".format(
                remote_file_name
            )
            ...
            xcopycommand = "xcopy /d /c /y {0} {1}".format(
                remote_file_name, local_dir_name
            )  # we copy to the local
            logging.info(xcopycommand)
            rc = os.system(xcopycommand)
            if rc != 0:
                logging.info(
