...
if __name__ == "__main__":
    ...
    if not os.path.exists(distributablep_filename):
        raise Exception(distributablep_filename + " does not exist")
    ...
    with open(distributablep_filename, mode="rb") as f:
        try:
            ...
            raise AttributeError(
                "An AttributeError when loading the pickle file is often caused by having the __main__ in the same file as a needed class. One possible fix is to add an import statement in the __main__ for the class. [Original message: '{0}'".format(
                    e
                )
            )
