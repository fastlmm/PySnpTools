...
from pysnptools.util import format_delta, _mbps
    ...
    **Constructor:**
    ...
    def __init__(self, common_directory, id_and_path_function, leave_space=0):
        ...
        self._str = "{0}('{1}',id_and_path_function=...{2}')".format(
            ...
            shared_directory_str,
            "" if self.leave_space == 0 else ",leave_space={0}".format(leave_space),
        )
    ...
    _copy_main_pattern = re.compile(r"^((main)|(copy_.*))\.txt$")
    _copy_pattern = re.compile(r"^copy_.*\.txt$")
    ...
    def _simple_open_read(self, simple_file_name, updater=None):
        ...
        try:
            ...
            for storage_path, storage_count in self._far_file_sequence(
                ...
                try:  # If something goes wrong, try the next one
                    ...
                    if (
                        psutil.disk_usage(os.path.dirname(local_path)).free - file_size
                        < self.leave_space
                        ...
                        for other_file in os.listdir(local_dir):
                            ...
                            if os.path.isfile(
                                ...
                                logging.info(
                                    "\tNeed space and file isn't in directory so removing it. '{0}'".format(
                                        full_file
                                    )
                                )
                                os.remove(full_file)
                    ...
                    delta_sec = max(
                        (datetime.datetime.now() - then).total_seconds(), 1.0
                    )
                    try:  # The 'try' stops this logging message from getting a div by zero error some times
                        logging.info(
                            "Copy time is {0}. Copy speed is {1} Mbps".format(
                                delta_sec, _mbps(file_size, delta_sec)
                            )
                        )
                    except Exception:
                        logging.info(
                            "Copy time is {0}. Copy speed can't be calculated Mbps".format(
                                format_delta(delta_sec)
                            )
                        )
                    break
            ...
            assert file_size == os.path.getsize(
                local_path
            ), "File did not copy or did not copy completely."
        ...
        assert dir_path._simple_file_exists(
            "main.txt"
        ), "File doesn't exist: '{0}'".format(dir_path)
    ...
    def _robust_load_main(self, dir_path):
        ...
        try:
            ...
            logging.info(
                "ignoring exception when doing net use on {0}, {1}".format(
                    main_path, e.message
                )
            )
        if os.path.exists(main_path):
            return main_path
        logging.info(
            "'{0}' does not exist, so looking for other copies.".format(main_path)
        )
        for copy_file in dir_path._simple_walk():
            ...
            if os.path.exists(copy_path):
                ...
                return copy_path
        raise Exception(
            "The main copy of the file can't be found and no replacement is available. '{0}'".format(
                main_path
            )
        )
    ...
    def _simple_open_write(self, simple_file_name, size=0, updater=None):
        ...
        dir_path = self.common_directory.join(simple_file_name)
        assert not dir_path._simple_file_exists(
            "main.txt"
        ), "Can't open a file for write if it already exists."
        assert not self._at_least_one(
            dir_path._simple_walk()
        ...
        if os.path.exists(local_path):
            logging.info(
                "File with the same name found locally, but without a directory entry, so removing it. '{0}'".format(
                    local_path
                )
            )
            os.remove(local_path)
        assert (
            psutil.disk_usage(os.path.dirname(local_path)).free - size
            > self.leave_space
        ), "Not enough space for '{0}. Need to make space".format(local_path)
        ...
        assert (
            psutil.disk_usage(os.path.dirname(local_path)).free > self.leave_space
        ), "Not enough space for '{0}'. Need to make space".format(local_path)
    ...
    def _remove_internal(self, path):
        for storage_item in self.common_directory.walk(path):
            ...
            if (
                storage_path != ""
            ):  # Dibs files don't point anywhere, so we don't need to delete the file they point to.
                try:
                    logging.debug("/tos.remove('{0}')".format(storage_path))
                    os.remove(
                        storage_path
                    )  # In storage, we remove the files, but not the folder
                except Exception:
                    logging.debug(
                        "Can't remove (because it's not there): '{0}'".format(
                            storage_path
                        )
                    )
    ...
    def _simple_remove(self, simple_file_name, updater=None):
        assert self._simple_file_exists(
            simple_file_name
        ), "Expect file to exist (and be a file)"
    ...
    def _simple_join(self, path):
        ...
        fs = PeerToPeer(
            self.common_directory._simple_join(path),
            id_and_path_function=closure,
