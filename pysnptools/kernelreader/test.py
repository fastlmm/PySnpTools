...
class TestKernelReader(unittest.TestCase):
    ...
    def test_merge_std(self):
        ...
        for std in [stdizer.Beta(2, 10), stdizer.Unit()]:
            ...
            snpreader = SnpData(
                ...
                val=np.array(
                    np.random.randint(3, size=[3, sid_count]),
                    dtype=np.float64,
                    order="F",
                ),
            )
            kerneldata0, trained0, diag0 = SnpKernel(
                snpreader, std, block_size=1
            )._read_with_standardizing(to_kerneldata=True, return_trained=True)
            kerneldata1, trained1, diag1 = SnpKernel(
                snpreader, std, block_size=None
            )._read_with_standardizing(to_kerneldata=True, return_trained=True)
            np.testing.assert_array_almost_equal(
                kerneldata0.val, kerneldata1.val, decimal=10
            )
            np.testing.assert_array_almost_equal(
                trained0.stats, trained1.stats, decimal=10
            )
            ...
            for dtype in [np.float64, np.float32]:
                ...
                for std in [stdizer.Unit(), stdizer.Beta(2, 10)]:
                    ...
                    snpreader1 = SnpData(
                        ...
                        sid=[str(i) for i in range(sid_count)],
                        val=np.array(
                            np.random.randint(3, size=[2, sid_count]),
                            dtype=dtype,
                            order=order,
                        ),
                    )
                    ...
                    for has_SNC_in_train in [False, True]:
                        ...
                        for has_missing_data in [False, True]:
                            ...
                            cppa, stdcppa = snpreader0.read(
                                ...
                                std, return_trained=True, force_python_only=False
                            )
                            pya, stdpya = snpreader0.read(
                                order=order, dtype=dtype
                            ).standardize(
                                std, return_trained=True, force_python_only=True
                            )
                            np.testing.assert_array_almost_equal(
                                cppa.val,
                                pya.val,
                            ...
                            if has_SNC_in_train:
                                assert np.array_equal(
                                    cppa.val[:, 1], np.zeros([cppa.val.shape[0]])
                                )
                                assert np.array_equal(
                                    pya.val[:, 1], np.zeros([pya.val.shape[0]])
                                )
                            ...
                            # uses stats
                            cppb = snpreader1.read(
                                order=order, dtype=dtype
                            ).standardize(stdcppa, force_python_only=False)
                            pyb = snpreader1.read(order=order, dtype=dtype).standardize(
                                stdpya, force_python_only=True
                            )
                            np.testing.assert_array_almost_equal(
                                cppb.val,
                                pyb.val,
                            ...
                            if has_SNC_in_train:
                                assert np.array_equal(
                                    cppb.val[:, 1], np.zeros([cppb.val.shape[0]])
                                )
                                assert np.array_equal(
                                    pyb.val[:, 1], np.zeros([pyb.val.shape[0]])
                                )
    ...
    def test_intersection(self):
        ...
        assert isinstance(k1.snpreader, _SnpSubset) and not isinstance(
            k1, _KernelSubset
        )
    ...
    def test_respect_read_inputs(self):
        ...
        for kernelreader in [
            SnpKernel(
                Bed("../examples/toydata.5chrom.bed", count_A1=True), StdIdentity()
            )[::2, ::2],
            Bed("../examples/toydata.5chrom.bed", count_A1=True)[::2, ::2].read_kernel(
                StdIdentity()
            ),
                ...
                for dtype in [np.float32, np.float64]:
                    for force_python_only in [True, False]:
                        for view_ok in [True, False]:
                            ...
                            if (
                                hasattr(kernelreader, "val")
                                and view_ok
                                and kernelreader.val is not val
                                and (
                                    order == "A"
                                    or (
                                        order == "F"
                                        and kernelreader.val.flags["F_CONTIGUOUS"]
                                    )
                                    or (
                                        order == "C"
                                ...
                                and (dtype is None or kernelreader.val.dtype == dtype)
                            ):
                                logging.info(
                ...
                for sid_count in [20, 2]:
                    ...
                    for stdx in [
                        ...
                        for snpreader0 in [snpdataX, snpdataX[:, 1:]]:
                            ...
                            ) = snpreader0.read().standardize(
                                stdx, return_trained=True, force_python_only=True
                            )
    ...
    def test_fail(self):
        ...
        assert (
            did_fail
        ), "The constructor should fail because the iid is the wrong shape"
    ...
    def test_kernel2(self):
        ...
        assert np.array_equal(
            kd.iid_to_index([["1", "1"], ["2", "2"]]), np.array([1, 2])
        )
        assert np.array_equal(
            kd.iid0_to_index([["1", "1"], ["2", "2"]]), np.array([1, 2])
        )
        kd = kd.standardize()
    ...
    def test_npz(self):
        ...
        np.testing.assert_array_almost_equal(
            kerneldata1.val, kerneldata2.val, decimal=10
        )
    ...
    def test_identity(self):
        ...
        assert (
            np.array_equal(kid.row, kid.iid)
            and np.array_equal(kid.iid, kid.iid0)
            and np.array_equal(kid.iid0, kid.iid1)
            and np.array_equal(kid.iid1, kid.col)
        )
        np.testing.assert_array_almost_equal(
            kid.read().val, np.identity(snpreader.iid_count)
        )
...
class TestKrDocStrings(unittest.TestCase):
    def test_kernelreader(self):
        ...
        result = doctest.testmod(
            pysnptools.kernelreader.kernelreader, optionflags=doctest.ELLIPSIS
        )
        os.chdir(old_dir)
    ...
    def test_snpkernel(self):
        ...
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        result = doctest.testmod(
            pysnptools.kernelreader.snpkernel, optionflags=doctest.ELLIPSIS
        )
        os.chdir(old_dir)
        assert result.failed == 0, "failed doc test: " + __file__
    ...
    def test_kerneldata(self):
        ...
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        result = doctest.testmod(
            pysnptools.kernelreader.kerneldata, optionflags=doctest.ELLIPSIS
        )
        os.chdir(old_dir)
        assert result.failed == 0, "failed doc test: " + __file__
    ...
    def test_identity(self):
        ...
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        result = doctest.testmod(
            pysnptools.kernelreader.identity,
    ...
    def test_kernelhdf5(self):
        ...
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        result = doctest.testmod(
            pysnptools.kernelreader.kernelhdf5, optionflags=doctest.ELLIPSIS
        )
        os.chdir(old_dir)
        assert result.failed == 0, "failed doc test: " + __file__
    ...
    def test_kernelnpz(self):
        ...
        os.chdir(os.path.dirname(os.path.realpath(__file__)))
        result = doctest.testmod(
            pysnptools.kernelreader.kernelnpz, optionflags=doctest.ELLIPSIS
        )
        os.chdir(old_dir)
        assert result.failed == 0, "failed doc test: " + __file__
